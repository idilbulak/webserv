...
// 			_ext = _req.getUri().path.substr(last_dot_pos);
// 			if (!_ext.empty()) {

// 				std::string newPath = "~\\" + _ext + "$";
// 				std::cout << "dsflhdfhkjs" << newPath << std::endl;
...
// 						// _path = _req.getUri().path.substr(0, last_slash_pos);
...
// 	if (!_file.empty() && _loc.path.at(0) != '~') {
// 		std::string path2;
// 		if (!_loc.root.empty())
// 			path2 = _loc.root + "/" + _file;
// 		else
// 			path2 = _file;
// 		if (!fileExists(path2)) {
// 			_code = 404;
...
// 				if (_ext.compare(_server.cgi_ext[i]) == 0)
// 					_cgiOn = 1;
// 			}
// 		// _cgiOn = 1;
//     }
